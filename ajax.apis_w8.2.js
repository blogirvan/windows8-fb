...
function bukaNavBar(){if($('#navbar').css('display')=='none'){$('.header-outer').hide('slow',function(){$('#navbar').show('slow')})}else{$('#navbar').hide('slow',function(){$('.header-outer').show('slow')})}}
function shutdown(){if($('#navbar').css('display')=='none'){$('#likebarucoy,.tabs-outer,.header-outer,.main-outer,#mta_bar,#iconup,#tutupdonk,#dialog-perhatian,#cboxdiv,.round3.black50.post-header-line-line,#jantung-irvan,#animasi-sm,#credit-ngisor,#jam-ngisor').slideUp(3500,function(){$('body').css('background','black url(http://www.physics.fsu.edu/courses/Summer12/PHY2054C/050203_wed_nosignal.gif)')})}else{$('#xx').hide('slow',function(){$('.xxx').hide('slow')})}}
...
if(post.feed.entry[x].author&&$('.keterangan-postingan .posted-by').length==0){
  if(post.feed.entry[x].author[0].gd$image){$('.gambar-author').html('<img class="round3" src="http://'+post.feed.entry[x].author[0].gd$image.src.split('\/\/')[1]+'" style="margin:0 5px 0 0;max-width:75px;max-height:75px;_width:75px"/>')}
  ...
  if(post.feed.entry[x].id){
    ...
    popComBt+='</div><div class="comment-button" style="display:inline-block;vertical-align:middle"><a class="tombol-fb" style="margin-left:0" href="http://web-game-sm.blogspot.com/" onClick="javascript:window.open(this.href,\'komeng\',\'width=400,height=600,scrollbars=yes\');return false">Comment</a></div><div style="display:inline-block;vertical-align:middle"><div class="nub"><s></s><i></i></div><div class="count">Donk</div></div>';
...
{"uri":"http://pelangi.faa.im/files/rt-wtwta-wild-rumpus-st.mp3","judul":"WTWTA - Wild Rumpus Start","loop":"no"}
    ...
    if(mp3ArrayAing[ke].loop){lop=mp3ArrayAing[ke].loop}
  }
  if(pley==null||pley==''||pley=='yes'||pley==1){pley='yes'}else if(pley=='no'||pley==0){pley='no'}else{if(judul==null||judul==''){judul=pley}pley='yes'}
  if(lop==null||lop==''||lop=='yes'||lop==1){lop='yes'}else if(lop=='no'||lop==0){lop='no'}else{if(judul==null||judul==''){judul=lop}lop='yes'}
  if(judul==null||judul==''){var purl=uri.split('/').length-1;var artist=uri.split('/')[2];var title=uri.split('/')[purl].split('.mp3')[0];judul=title}else{var artist=judul.split(' - ')[0];if(judul.split(' - ')[1]){var title=judul.split(' - ')[1]}else{var title=judul}}
  var album=window.location.hostname; 
  var srcmp3='http://beemp3.com/player/player.swf?soundFile='+uri;
  srcmp3+='&autostart='+pley;
  srcmp3+='&loop='+lop;
  srcmp3+='&bg=0x000000&leftbg=0x4B0082&lefticon=0xF2F2F2&rightbg=0xD67919&rightbghover=0x1BAD07&righticon=0xF2F2F2&righticonhover=0xFFFFFF&text=0xE0E3F4&slider=0x357DCE&track=0xFFFFFF&border=0xFFFFFF&loader=0xAF2910';
  ...
  tulisNew+='<div style="width:30px;height:20px;overflow:hidden;display:inline-block;vertical-align:middle"><div><embed class="embed-music" wmode="transparent" src="'+srcmp3+'" quality="high" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" align="top" width="230" height="21"></embed></div></div>';
...
if(gbrUserYgLogin==null||gbrUserYgLogin==''){gbrUserYgLogin='https://6a69cede-a-62cb3a1a-s-sites.googlegroups.com/site/irvancyber4rt/irvan-cyber4rt/vanz1.jpg'}
$('.Profile .widget-content').html('<a class="a-profile-img" target="_blank"><img class="shadow profile-img" src="'+gbrUserYgLogin+'" style="width:100%;height:auto;float:none"/></a>'+profImgLikeBtn);
